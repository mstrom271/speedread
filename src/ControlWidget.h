...
class ControlWidget : public QWidget {
  ...
  public:
    ControlWidget(QWidget *wgt = nullptr) : QWidget(wgt) {
        // clang-format off
    ...
    int getSpeed() { return speedSlider->sliderPosition(); }
  ...
  public slots:
    ...
    void setBookWidthSilent(int newBookWidth) {
        ...
        bookWidthLabel->setText(
            QString::number(bookWidthSlider->sliderPosition()));
        bookWidthSlider->blockSignals(false);
    }
    ...
    void setFragSizeSilent(int newFragSize) {
        ...
        fragSizeLabel->setText(
            QString::number(fragSizeSlider->sliderPosition()));
        fragSizeSlider->blockSignals(false);
    }
    ...
    void resizeEvent(QResizeEvent *event) override {
        for (auto wgt : std::initializer_list<QSlider *>{
            ...
            wgt->setMinimumHeight(
                std::min(QApplication::primaryScreen()->size().height(),
                         height()) /
                3);
            ...
            QString slider_halfsize = QString::number(
                static_cast<int>(wgt->width() * (wgt_size / 2)));
            QString slider_quartsize = QString::number(
                static_cast<int>(wgt->width() * (wgt_size / 4)));
        ...
        for (auto wgt : std::initializer_list<QPushButton *>{
                 playBtn, openBtn, bookPosBtn, bookWidthBtn, fragSizeBtn,
                 speedBtn, settingsBtn}) {
